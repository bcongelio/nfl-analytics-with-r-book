...
xyac_meanyardage <- pbp %>%
  ...
  group_by(receiver) %>%
  mutate(mean_yardage_result = ifelse(yards_after_catch >= xyac_mean_yardage, 1, 0)) %>%
  summarize(total_receptions = sum(complete_pass == 1, na.rm = TRUE),
            total_higher = sum(mean_yardage_result, na.rm = TRUE),
...
```{r pass-oe-dictionary, eval = TRUE, echo = TRUE, output = TRUE, fig.align='center', fig.dpi = 400}
...
ggplot(pass_over_expected, aes(x = ydstogo, y = mean_passoe, group = posteam)) +
