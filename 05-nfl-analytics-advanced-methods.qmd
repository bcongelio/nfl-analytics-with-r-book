...
To start dropping predictor variables, we will first start by using problematic correlation before doubling-back to separation issues. Despite providing an overwhelming look at the relationship between our predictor variables, the correlation visualization does suggest a few clear variables to drop.
...
A multinomial regression model is used when exploring the relationship between predictor variables and a dependent variables that has more than two levels. The model itself ultimately determines the probability of each dependent variable based on the given set of predictive values. The core underlying the main assumptions of th model are:
...
model_data <- pbp_prep %>%
  ...
  select(-play_id, -game_id, -game_date, -week,
         -play_type, -yards_gained, -run, -pass, -air_yards, -run_location)
```
...
With data preparation and feature engineering complete, we can turn to constructing the model within the `tidymodels` framework.
...
```{r multinomial-recipe, eval = FALSE, echo = TRUE, output = FALSE}
...
```{r multinomial-tune-results, eval = FALSE, echo = TRUE, output = FALSE}

`> multinom_play_tune
# A tibble: 10 x 5
   splits                 id     .metrics          .notes           .predictions           
   <list>                 <chr>  <list>            <list>           <list>                 
 1 <split [349579/38845]> Fold01 <tibble [40 x 5]> <tibble [0 x 3]> <tibble [776,900 x 10]>
 2 <split [349579/38845]> Fold02 <tibble [40 x 5]> <tibble [0 x 3]> <tibble [776,900 x 10]>
 3 <split [349580/38844]> Fold03 <tibble [40 x 5]> <tibble [0 x 3]> <tibble [776,880 x 10]>
 4 <split [349581/38843]> Fold04 <tibble [40 x 5]> <tibble [0 x 3]> <tibble [776,860 x 10]>
 5 <split [349581/38843]> Fold05 <tibble [40 x 5]> <tibble [0 x 3]> <tibble [776,860 x 10]>
 6 <split [349582/38842]> Fold06 <tibble [40 x 5]> <tibble [0 x 3]> <tibble [776,840 x 10]>
 7 <split [349583/38841]> Fold07 <tibble [40 x 5]> <tibble [0 x 3]> <tibble [776,820 x 10]>
 8 <split [349583/38841]> Fold08 <tibble [40 x 5]> <tibble [0 x 3]> <tibble [776,820 x 10]>
 9 <split [349584/38840]> Fold09 <tibble [40 x 5]> <tibble [0 x 3]> <tibble [776,800 x 10]>
10 <split [349584/38840]> Fold10 <tibble [40 x 5]> <tibble [0 x 3]> <tibble [776,800 x 10]>
`
...
```{r multinom-show-best, eval = FALSE, echo = TRUE, output = FALSE}
...
1 0.00001   roc_auc hand_till  0.668    10 0.000468 Preprocessor1_Model01
2 0.0000183 roc_auc hand_till  0.668    10 0.000468 Preprocessor1_Model02
3 0.0000336 roc_auc hand_till  0.668    10 0.000468 Preprocessor1_Model03
4 0.0000616 roc_auc hand_till  0.668    10 0.000468 Preprocessor1_Model04
5 0.000113  roc_auc hand_till  0.668    10 0.000468 Preprocessor1_Model05`
```

ffff

```{r multinom-best-and-last-fit, eval = FALSE, echo = TRUE, output = FALSE}
...
`# A tibble: 2 x 4
  ...
  <chr>    <chr>          <dbl> <chr>               
1 accuracy multiclass     0.425 Preprocessor1_Model1
2 roc_auc  hand_till      0.667 Preprocessor1_Model1`
```

ffff

```{r multinom-pulling-workflow-merge, eval = FALSE, echo = TRUE, output = FALSE}
