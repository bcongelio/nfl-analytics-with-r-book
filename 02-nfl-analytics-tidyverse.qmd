...
In other cases, we may need to do the opposite of above - that is, select all teams *except* for specific ones. For example, using data from Pro Football Reference regarding a quarterback's average time to throw, let's determine how to *remove* all AFC North teams (Pittsburgh, Cleveland, Baltimore, and Cincinnati) from data while leaving all other teams intact. Such a procedures is necessary, for example, if we wanted to explore the average time to throw for the combined AFC North teams against the rest of the NFL.
...
```{r removing-afc-north-teams}

```

Because of the structure of the logical operators, we are able to construct a single `filter()` combined with various operator requests. For example, let's gather only those rows that fit the following specifications:
...
How many total scrimmage yards did NFL teams earn during the 2022 NFL regular season? According to the results from our above code, the answer is 184,444. However, that is an incorrect answer.
...
[![Mutating Joins in dplyr](docs/images/mutating_joins_picture.png){fig-align="center" width="400"}](https://rpubs.com/williamsurles/293454)
