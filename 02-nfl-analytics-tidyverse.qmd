...
The output, unfortunately, is not all that helpful. Because we did not use the `filter` verb to stipulate a minimum number of rushing attempts, the output is saying that - for example, Aaron Rodgers, a quarterback, has among the most successful rushers in the league with a 100% rushing success rate. To correct this, we must add a second metric to our `summarize` verb (we will call it `n_rushes`) and then use the `filter` verb afterwards to include a minimum number of rushes required to be included in the final output.
...
Unsurprisingly, Miles Sanders - a running back for the Eagles, who lead the NFL in team success rate - is the leader in rushing success among individual players with 50% of his attempts gaining positive EPA.
...
The Las Vegas Raiders, based on our definitions, are the most aggressive passing team in the league on 3rd and short as just over 81% of their air yards were at - or past - the required yardage for a first down. On the other end of the spectrum, the New York Giants are the least aggressive team in the NFL, at 49%.
