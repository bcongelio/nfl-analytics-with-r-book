...
ggplot(data = qb_thirddown_data, aes(x = reorder(passer_id, -qb_agg_pct), y = qb_agg_pct)) +
  ...
  with_outer_glow(geom_text(aes(label = passer), position = position_stack(vjust = .98),
                            ...
                            family = "Roboto Condensed", fontface = "bold", size = 8),
...
It should not be a surprise that the Chiefs outperfor - by a large margin - the rest of the NFL. The Texans and Colts, on the other hand, averaged -0.15 expected points added per play during the 2022 regular season.
...
complete_wr_airyards <- vroom::vroom("url here")
...
pure_cpoe <- vroom::vroom("https://raw.githubusercontent.com/bcongelio/nfl-analytics-with-r-book/origin/example_data/csv/pure_cpoe.csv")
```
