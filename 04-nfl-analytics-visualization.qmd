...
play_action_data <- vroom("https://raw.githubusercontent.com/bcongelio/nfl-analytics-with-r-book/origin/example_data/csv/play_action_data.csv")
```
...
While the our current plot includes team-specific colors for the points, we are still not able to discern - for the most part - which player belongs to which point. To rectify this, we will turn to using the `ggrepel` package, which is designed to improve the readability of text labels on plots by automatically repelling overlapping labels, if any. `ggrepel` operates out of two main functions: `geom_text_repel` and `geom_label_repel`. Both provide the same end result, with the core different being `geom_label_repel` adding a customized label under each player's name.
...
ggplot(data = play_action_data, aes(x = yds, y = pa_yds)) +
  ...
  theme(
    axis.ticks = element_blank(),
    axis.title = element_text(family = "Overpass",
                              size = 10, 
                              color = "black"),
    ...
    plot.title = element_text(family = "Overpass",
                              ...
                              vjust = .02,
                              hjust = 0.5),
    plot.subtitle = element_text(family = "Overpass",
                                 size = 12,
                                 color = "black",
                                 hjust = 0.5),
    plot.caption = element_text(family = "Overpass",
                                size = 8,
                                face = "italic",
                                color = "black"),
...
nfl_analytics_theme <- function(..., base_size = 12) {
    ...
    theme(
      text = element_text(family = "Overpass", size = base_size),
      axis.ticks = element_blank(),
...
team_playaction <- vroom("https://raw.githubusercontent.com/bcongelio/nfl-analytics-with-r-book/origin/example_data/csv/team_playaction.csv")
...
```{r eagles-line-3, output = TRUE, fig.align='center', fig.dpi = 400}
...
```{r mahomes_epa_1, output = TRUE, fig.align='center', fig.dpi = 400}
...
```{r mahomes_epa_2, output = TRUE, fig.align='center', fig.dpi = 400}
...
```{r mahomes_epa_3, output = TRUE, fig.align='center', fig.dpi = 400}
...
To start, we will place `passer_id` on the x-axis and `qb_agg_pct` on the y-axis. However, to make sure the columns are plotted in descending order, we use the `reorder()` function to reorder `passer_id` in descending order by the `qb_agg_pct` variable.
...
ggplot(data = qb_thirddown_data, aes(x = reorder(passer_id, -qb_agg_pct), y = qb_agg_pct)) +
  ...
  with_outer_glow(geom_text(aes(label = passer), position = position_stack(vjust = .98),
                            ...
                            family = "Overpass", fontface = "bold", size = 8),
...
```{r geom_col_final, output = TRUE, fig.align='center', fig.dpi = 400}
...
As highlighted in the resulting plot, Tagovailoa was the only QB in the league to go above 60% in aggressiveness (Derek Carr was an even 60%). In this case, our domain knowledge of the NFL is important as we will recall that Tua was out for portions of the 2022 regular season as a result of concussions. His aggressive percentage could be slightly inflated because of the fewer games compared to the other quarterbacks.
...
The `ggplot()` code required to output the visulization is quite similar to our above `geom_col()` plot for QB aggressiveness. There are several distinct differences, however.
...
We can create a `geom_box()` plot to examine the distribution of air yards for each playoff team from the 2022 season. A box plot is a suitable way to, as mentioned, view distribution but to also view the "skewness" of a metric by also including the interquartile ranges, averages, and any outliers in the data.
...
playoff_airyards_ridges_plot <- vroom("https://raw.githubusercontent.com/bcongelio/nfl-analytics-with-r-book/origin/example_data/csv/playoff_airyards_ridges_plot.csv")
```
...
The `meansd` function is included in the `geom_density_ridges()` arguments. We do wish to show `quantile_lines`, so we provide `meansd` to the `quantile_fun` argument and then set the `color` of the quantile lines.

```{r finished_geom_ridges, output = TRUE, fig.align='center', warning = FALSE, fig.dpi = 400}
...
yards_gained_compared <- vroom("https://raw.githubusercontent.com/bcongelio/nfl-analytics-with-r-book/origin/example_data/csv/yards_gained_compared.csv")
```

```{r violin_plot_completed, output = TRUE, fig.align='center', warning= FALSE, fig.dpi = 400}
...
ggplot(data = yards_gained_compared, aes(x = posteam, y = yards_gained, group = posteam, fill = posteam)) +
  ...
  theme(axis.text.x = nflplotR::element_nfl_logo(size = 1.5),
        ...
        strip.text = element_text(family = "Overpass", face = "bold"),
...
def_yards_allowed <- vroom("https://raw.githubusercontent.com/bcongelio/nfl-analytics-with-r-book/origin/example_data/csv/def_yards_allowed.csv")
```
...
The next step, which is a modified version of an approach used by [Tanya Shapiro](https://twitter.com/tanya_shapiro), is multifaceted. We first use the `magick` package to create a composite of the image and place a white background behind the circular cropped headshot.
...
plot_image_label<-function(image,
  ...
  ggplot() +
    ...
    geom_textpath(data = data, aes(x,y,label = toupper(label)), linecolor=NA, color=font_color,
                  size = 16,  fontface="bold", vjust = vjust, hjust=hjust, family = "Overpass")+
    coord_equal()+
    theme_void()
}
```
...
pure_cpoe <- vroom("https://raw.githubusercontent.com/bcongelio/nfl-analytics-with-r-book/origin/example_data/csv/pure_cpoe.csv")
```

As mentioned, we can quickly take the data and turn it into a `gt` table by piping it directly into the package's core function.

```{r pure-cpoe-straight-pipe, eval = TRUE, echo = TRUE, output = TRUE}

pure_cpoe %>%
  gt()
```

In some cases, the results might not be *terrible* ... but, in this case, it is. The output is realistically just the data frame outputted into a different format, including the complete URLs for both the player headshot and team logos. Let's begin making incremental changes to the table by first adding a title and subtitle, using Markdown (`md()`) to provide bold and italicized styling to each.

```{r pure-cpoe-tab-header, eval = TRUE, echo = TRUE, output = TRUE}
...
Adding the title and subtitle to the top of the table may not help much, as the two long-form URLs are stretching out the table and making it difficult to see the changes that are being made. Rather than including both the player's picture and the team logo, let's remove `team_logo_wikipedia` from being used by `gt` with the `cols_hide()` function and then place the player pictures into table by using `gt_img_rows()` from the `gtExtras` package.

```{r remove-wiki-add-photo, eval = TRUE, echo = TRUE, output = TRUE}
...
```{r add-stub-head-label, eval = TRUE, echo = TRUE, output = TRUE}
...
```{r moving-images-and-align, eval = TRUE, echo = TRUE, output = TRUE}
...
ryoe_shiny <- vroom("https://raw.githubusercontent.com/bcongelio/nfl-analytics-with-r-book/origin/example_data/csv/ryoe_projs.csv")
```
...
ui <- fluidPage(
  ...
  sidebarLayout(fluid = TRUE,
    sidebarPanel(
      ...
      sliderInput("down", "Down:", min = min(ryoe_shiny$down),
                  max = max(ryoe_shiny$down), value = range(ryoe_shiny$down)),
      sliderInput("ydstogo", "Yards to Go:", min = min(ryoe_shiny$ydstogo),
                  max = 10, value = range(ryoe_shiny$ydstogo)),
