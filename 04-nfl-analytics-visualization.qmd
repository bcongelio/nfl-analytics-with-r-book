...
Not all types of data and examination of relationships will work with all the various types of plots within the `ggplot2` family of packages. In general, much of the NFL data you work with will explore correlations, deviation, rankings, and change. A `geom_point()` plot will not work well with deviation (while a `geom_boxplot()` will) and vice versa.

The below flowchart will help you In determining which type of plot works best for the data you are working with.
...
Please note that difference in how the `breaks` on each scale was handled. Because we want each year on the x-axis, we manually controlled the output by using the `seq()` function to start the x-axix at `2012` and to increase by `1` until the final number of `2022` is reached. On the other hand, we used the `pretty_breaks()` function from `scales` to format the number of breaks on the y-axis and then used `percent_format()` to edit the labels to be whole numbers with the percentage sign included.
