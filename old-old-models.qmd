...
**However, we are going to use this failure to examine another approach to regression in the next section: Penalized Regression Techniques**. By using a ridge or lasso regression, the model will either "shrink" the coefficients of those features found to be less important or by controlling their size against the overall numbers in the model. We will revert back to our original model data (before removing variables based on correlation and separation) to see if using a penalized regression technique can do a better job at removing/handling problematic predictor variables than we did.
:::

#### Multinomial Regression using `tidymodels`
...
As a result, you will notice I resorted to using the `fcase()` function from the `data.table()` package. You can also use the `dt_case_when()` function from `tidyfast` if you prefer to use the same syntax of `case_when()` but utilize the speed of `data.table::fielse()`.
...
With the `initial_split()` process completed, the data is divided into a `qb_mvp_training` data frame and a `qb_mvp_testing` data frame using the `training()` and `testing()` functions within `rsample` to conduct the process on the `` `qb_mvp_split` `` data.
...
```{r multinomial-model-wf, eval = FALSE, echo = TRUE, output = FALSE}
...
```{r loading-multinom-results, eval = TRUE, echo = FALSE, output = FALSE}
...
```{r multinom-roc_auc_curve, echo = TRUE, output = TRUE, message = FALSE, fig.align='center', fig.dpi = 400}
...
```{r loading-multinom-teams, eval = TRUE, echo = FALSE, output = FALSE}
...
```{r multinom-most-pred-teams, eval = TRUE, echo = TRUE, output = FALSE}
...
```{r multinom-team-chart, eval = TRUE, echo = TRUE, output = TRUE, message = FALSE}
