...
###testing <- nflreadr::load_pbp(2021) run and upload

###testing <- testing %>%
  filter(season_type == "REG") %>%
  select(play_id, game_id, posteam, passer, passer_player_id, passer_player_name,
         play_type, pass, down, ydstogo, air_yards, sack, interception,
         complete_pass, incomplete_pass)###run and upload

completions_airyards <- testing %>%
...
```{r correlation-removing-adalton-jfields}
