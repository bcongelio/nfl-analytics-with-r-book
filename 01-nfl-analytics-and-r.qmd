...
Because we want to determine if the elusiveness of a team's stable of running backs directly impacts the offensive line's ability to created Adjusted Line Weights, we can create a new variables that uses the team's `elusive_rating` to statistically weight the `adj_line_yards`. While there are numerous approaches to doing so (such as ranking, categorical weighting, logarithmic weighting, etc.), we will conduct a linear transformation wherein we scale/normalize the `avg_elu` variable and then subtract that from each team's `adj_line_yards`. As a result, those teams with high `avg_elu` value will result in lower `adj_line_yards`, as the running back is successful despite the performance the offensive line in front of him.

```{r plotting-weighted-elusive, output = TRUE, fig.align='center', fig.dpi = 400}

oline_combo_elusive <- oline_combo_elusive %>%
  mutate(avg_elu = scale(avg_elu)) %>%
  mutate(adj_line_yards_weighted = adj_line_yards - (avg_elu / max(avg_elu)))
  
ggplot(oline_combo_elusive, aes(x = combos, y = adj_line_yards_weighted)) +
  geom_smooth(method = "lm", se = FALSE, size = 0.8, alpha = .08) +
  nflplotR::geom_mean_lines(aes(v_var = combos, h_var = adj_line_yards_weighted),
                            color = "black", size = .8) +
  nflplotR::geom_nfl_logos(aes(team_abbr = team), width = 0.065) +
  scale_x_reverse(breaks = scales::pretty_breaks()) +
  scale_y_continuous(breaks = scales::pretty_breaks(),
  ...
  labs(title = "**Unique Offensive Line Combinations vs. Weighted Adjusted Line Yards**",
       subtitle = "*2022 Season  |  ALY Weighted by PFF's Elusive Rating*",
       caption = "*An Introduction to NFL Analytics with R*<br>**Brad J. Congelio**") +
  xlab("# of Unique Offensive Line Combinations") +
...
The results are quite promising in that both the Los Angeles Rams and the Arizona Cardinals, with the most unique offensive line combinations in the league, have the third and fourth-lowest weighted adjusted line yards, while the teams with the lowest number of combinations have among the highest weighted ALY (such as the Carolina Panthers, Atlanta Falcons, and Philadelphia Eagles). There are clearly some outliers in the results in the Denver Broncos and Washington Commanders.
