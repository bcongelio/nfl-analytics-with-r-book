...
To continue this exploration, let's pivot away from exploring the impact of offensive line combinations on the quarterback and examine any potential correlation with the running game and run blocking. We can gather statistics provided by [Football Outsiders](https://www.footballoutsiders.com/) concerning the performance of offensive lines:
...
You can see in the output of the data that `power_rank` and `stuffed_rank` are both in a "ranked format," meaning the team with the best power success score (the Pittsburgh Steelers) are ranked 1 while the team with the worst (the Minnesota Vikings) are ranked 32. The `adj_line_yards` variable is provided in its unranked, raw format. Because of this, let's first both `power_rank` and `stuffed_rank` and use the `cowplot` package to view them together, and then construct the plot for `adj_line_yards` separately since it operates on a differing y-axis scale.
...
Despite have two of the bottom four scores in `elusive_rating` (21.4 and 21.8), the Rams and the Cardinals are in the league's top five for `Weighted Adjusted Line Yard`. This is an indication that each team's offensive line proficient at run blocking and that any perceived shortcoming in the ground game was the result of a lack of elusive running backs. Conversely, the Washington Commanders unweighted `Adjusted Line Yards` was 4.29 but, when account for the high elusiveness score among the team's running backs, the metric dipped to a league-worst 2.14 weighted adjusted line yards.

It is notable that, despite the assumption that a high number of offensive line combinations throughout a season will result in poor metrics, the results of this brief data analysis concludes otherwise.

While there is certainly more examination that could be conducted, the early results seem to indicate that a higher amount of unique offensive line combinations impacts the rushing game more than the passing game - and, even then, there are caveat within that assumption because we must consider the talent level of the running back that is behind each offensive line.
