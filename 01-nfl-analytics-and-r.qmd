...
With the data cleaned and prepared, let's use it to create a `ggplot` graph and, to do so, compare the relationship between a team's number of unique offensive lines against its winning percentage. To complete this, we first need to join the winning percentages to our existing `total_combos` data frame.
...
```{r plot-combos, output = TRUE, fig.align='center', fig.dpi = 400}

extrafont::loadfonts(device = "win")

ggplot(data = total_combos, aes(x = combos, y = win_pct)) +
  geom_line(stat = "smooth", method = "lm", size = .7, color = "blue", alpha = 0.25) +
...
```{r plot-pressure-rate, output = TRUE, fig.align='center', fig.dpi = 400}
...
To begin, we can collect the QB hit data from `nflreadr` being sure to `group_by` the `posteam` variable in order to calculate the number of QB hits allowed by each team's offensive line. After, we can merge the data into our existing `total_combos` data set and then produce the plot.
...
After first filtering out any data that does not include `posteam` information, we can `group_by` each individual offensive unit and then calculate the total sum of QB hits for each. We can then take our new data frame titled `qb_hits_combined` and plot the results.

```{r plot-combos-qb-hits, output = TRUE, fig.align='center', fig.dpi = 400}
...
Let's read in the data and then create the data visualization.

```{r asr-data-for-plot, eval = TRUE, echo = TRUE, output = FALSE}

adjusted_sack_rate <- vroom("https://raw.githubusercontent.com/bcongelio/nfl-analytics-with-r-book/origin/example_data/csv/adjusted_sack_rate.csv")
```

```{r asr-2022-plot, output = TRUE, fig.align='center', fig.dpi = 400}
...
adjusted_sack_rate %>%
  ...
  labs(title = "**Unique Offensive Line Combinations vs. Adjusted Sack Rate**",
       subtitle = "*2022 Season*",
       caption = "*An Introduction to NFL Analytics with R*<br>**Brad J. Congelio**") +
  nfl_analytics_theme()
```
...
```{r asr-facet-plot, output = TRUE, fig.align='center', fig.dpi = 400}
...
To continue this exploration, let's pivot away from exploring the impact of offensive line combinations on the quarterback and examine any potential correlation with the running game and run blocking, while using metrics that are adjusted such as sack rate, as the additional context and weighting seemed to be helpful in finding relationsips. We can gather statistics provided by [Football Outsiders](https://www.footballoutsiders.com/) concerning the performance of offensive lines.

### Unique Offensive Line Combinations vs. Adjusted Line Yards

1.  **Adjusted Line Yards** - derived by Football Outsiders with a regression analysis, this metric takes the total of a team's total rushing attempts and assigns a quantitative value to the offensive line's impact. Any run that ends of a loss of yards results in 120% of the value being placed on the offensive line, while a 0-4 yard rush is 100%, a 5-10 yard rush is 50%, and anything over 11 yards is a 0% value. As explained by Football Outsiders, if a running back breaks free for a 50-yard gain, just how much of that is the offensive line responsible for? The adjusted line yards metric is a stable metric that can provide a numeric value as an answer.
...
```{r fb-outsiders-data, eval = TRUE, echo = TRUE, output = TRUE}

fb_outsiders_oline <- vroom("https://raw.githubusercontent.com/bcongelio/nfl-analytics-with-r-book/origin/example_data/csv/fboutsiders_line_stats.csv")

fb_outsiders_oline
```

You can see in the output of the data that `power_rank` and `stuffed_rank` are both in a "ranked format," meaning the team with the best power success score (the Pittsburgh Steelers) are ranked 1st while the team with the worst (the Minnesota Vikings) are ranked 32nd. The `adj_line_yards` variable is provided in its unranked, raw format. Because of this, let's first both `power_rank` and `stuffed_rank` and use the `cowplot` package to view them together, and then construct the plot for `adj_line_yards` separately since it operates on a differing y-axis scale.

```{r power-and-stuff-cowplot, output = TRUE, fig.align='center', fig.dpi = 400}

power_rank_plot <- ggplot(fb_outsiders_oline, aes(x = combos, y = power_rank)) +
  nflplotR::geom_mean_lines(aes(v_var = combos, h_var = power_rank),
                            color = "black", size = .8) +
  nflplotR::geom_nfl_logos(aes(team_abbr = team), width = 0.065) +
  scale_x_reverse(breaks = scales::pretty_breaks()) +
  scale_y_reverse(breaks = seq(1, 32, 2)) +
  labs(title = "**Unique Offensive Line Combinations vs. Power Ranking**",
...
stuffed_rank_plot <- ggplot(fb_outsiders_oline, aes(x = combos, y = stuffed_rank)) +
  geom_smooth(method = "lm", se = FALSE, size = 0.8, alpha = .08) +
  nflplotR::geom_mean_lines(aes(v_var = combos, h_var = stuffed_rank),
                            color = "black", size = .8) +
  nflplotR::geom_nfl_logos(aes(team_abbr = team), width = 0.065) +
  scale_x_reverse(breaks = scales::pretty_breaks()) +
  scale_y_reverse(breaks = seq(1, 32, 2)) +
  labs(title = "**Unique Offensive Line Combinations vs. Stuffed Rank**",
       subtitle = "*2022 Season  |  FootballOutsiders.com*",
       caption = "*An Introduction to NFL Analytics with R*<br>**Brad J. Congelio**") +
  xlab("# of Unique Offensive Line Combinations") +
...
ggplot(fb_outsiders_oline, aes(x = combos, y = adj_line_yards)) +
  ...
  labs(title = "**Unique Offensive Line Combinations vs. Adjusted Line Yards**",
       subtitle = "*2022 Season  |  FootballOutsiders.com*",
       caption = "*An Introduction to NFL Analytics with R*<br>**Brad J. Congelio**") +
  xlab("# of Unique Offensive Line Combinations") +
...
oline_combo_elusive <- vroom("https://raw.githubusercontent.com/bcongelio/nfl-analytics-with-r-book/origin/example_data/csv/online_combo_elusive.csv")
```
...
```{r plotting-weighted-elusive, output = TRUE, fig.align='center', fig.dpi = 400}
...
ggplot(oline_combo_elusive, aes(x = combos, y = weighted_aly)) +
  ...
  scale_x_reverse(breaks = scales::pretty_breaks()) +
  scale_y_continuous(breaks = scales::pretty_breaks()) +
  labs(title = "**Unique Offensive Line Combinations vs. Weighted Adjusted Line Yards**",
...
Despite have two of the bottom four scores in `elusive_rating` (21.4 and 21.8), the Rams and the Cardinals are in the league's top five for `Weighted Adjusted Line Yard`. This is an indication that each team's offensive line is proficient at run blocking and that any perceived shortcoming in the ground game was the result of a lack of elusive running backs. Conversely, the Washington Commanders unweighted `Adjusted Line Yards` was 4.29 but, when account for the high elusiveness score among the team's running backs, the metric dipped to a league-worst 2.14 weighted adjusted line yards.
...
In November of the 2022 NFL season, Warren Sharp - of [Sharp Football Analysis](https://www.sharpfootballanalysis.com/) - tweeted perhaps the most ridiculous statistic thus far into the season.
...
```{r explosive-play-calc, eval = TRUE, echo = TRUE, output = FALSE}
...
After collecting the play-by-play information for the 2022 NFL season, we use `filter()` to gather only those `fixed_drive_result` rows include `Touchdown` and then remove any play that is a special teams play and includes a fumble or interception. This is done as we only want those touchdowns scored by the offense, not a fumble or interception return for a score. Because of this, the results of total touchdowns is lower than the "official" number from Pro Football Reference, as the site's "Touchdown Log" includes this data.
...
```{r explosive-play-plot, output = TRUE, fig.align='center', fig.dpi = 400}
...
ggplot(explosive, aes(y = reorder(posteam, percent_w_exp), x = percent_w_exp)) +
  ...
  labs(title = "**Which Team Has The Highest % of Explosive Plays per TD Drive?**",
       subtitle = "*2022 Season*",
       caption = "*An Introduction to NFL Analytics with R*<br>**Brad J. Congelio**") +
