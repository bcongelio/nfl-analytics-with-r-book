...
Was Tom Brady near the top of the completion leader board throughout the 2021 NFL season? Or did he perhaps jump atop the leader board with an above-average number of completions in the final weeks of the season?
...
First, we are using the `filter` function from `dplyr` to calculate completed passes from just the regular season. In order to calculate for each individual quarterback, we are using the `group_by` function to lump all distinct `player_id` together. Afterwards, we are creating a new column within the data frame using the `mutate` function and titling it `all_completions` which is the sum of all complete passes for each quarterback. This is done in order to efficiently use the `filter` function to limit the results to just the top ten quarterbacks by total completions. Lastly, we use the `summarize` function to distill the data down to four columns. Of importance is the `total_completions` column we create using the `cumsum` function (ie., cumulative total), which adds each QB's completions week-by-week for a running total. After, we grab the `week`, `name`, and `posteam` columns for the purposes of data visualization.

You can see the first ten rows of the data by simply typing the name of the data frame and running it:

```{r viewing-week-by-week-results, echo = TRUE}

week_by_week
```

With the data manipulation complete, let's take the information and visualize it by cumulative completions by week for each quarterback. But, first, to allow for differentiation, lets bring in team colors into the data frame using the `team_colors_logos` function within the `nflfastR` package:
...
It is clear that Tom Brady and Patrick Mahomes started pulling ahead of other NFL QBs starting in approximately week seven. The two alternated as the NFL completion leader from week nine until week 12 which, at that point, Brady maintained a steady trajectory while Mahomes fell into a tightly packed race for second place for the remainder of the season.

While the graph, as is, provides insights to our inquiry it is certainly a bit on the busy side. What if we wanted to highlight *just* Brady and Mahomes? We can use the same plot we just created, but use the `alpha` function to control the opacity of the lines.

```{r controling-alpha-for-geom-lines, include = TRUE}

ggplot(data = week_by_week, aes(x = week, y = total_completions, group = player_id,
                                alpha = player_id %in% c("00-0033873", "00-0019596"))) +
  geom_line(color = week_by_week$team_color, size = 1.5) +
  scale_alpha_manual(values = c(0.05, 1), guide = "none") +
  geom_point(color = week_by_week$team_color2, size = 1) +
  scale_x_continuous(breaks = seq(1,18,1),
                     expand = c(0,0)) +
  scale_y_continuous(breaks = scales::pretty_breaks()) +
  ...
  ylab("Total Completions") +
  xlab("Week") +
  labs(title = "Cumulative Completions per Week | T.Brady & P.Mahomes Highlighted",
       subtitle = "2021 Season")

```

![](_book/images/completed-passes-brady-mahomes.png){fig-align="center"}

By adding `alpha` to the aesthetics call within `ggplot`, we are able to decrease the opacity for QB line on the chart that is not Tom Brady or Patrick Mahomes. This approach allows for a more direct view of the battle between the two, especially through weeks 9 and 12.

As mentioned, there are several influences that can dictate the total number of completions that a quarterback makes. To highlight this, we can add an additional variable to compare against total completions - in this case, air yards.

### Total Completions vs. Air Yards

To start, lets load in the data:

```{r completions_airyards_data, include = TRUE, output = FALSE}

completed_passes <- readr::read_csv("https://raw.githubusercontent.com/bcongelio/nfl-analytics-with-r-book/origin/example_data/completions_airyards.csv")

```

```{r completed-passes-vs-air-yards, echo = TRUE}

completions_airyards <- completions_airyards %>%
  group_by(passer_player_id) %>%
  filter(complete_pass == 1, !is.na(down)) %>%
  summarize(name = passer_player_name,
            posteam = last(posteam),
            completions = sum(complete_pass == 1, na.rm = TRUE),
            total_airyards = sum(air_yards, na.rm = TRUE),
            mean_airyards = total_airyards / completions) %>%
  distinct(passer_player_id, .keep_all = TRUE) %>%
  filter(completions >= 149) %>%
  left_join(teams_colors_logos, by = c("posteam" = "team_abbr"))
```

stuff here then graph

```{r completed-passes-vs-air-yards-graph, echo = TRUE, eval = FALSE}

ggplot(data = completions_airyards, aes(x = completions, y = mean_airyards)) +
  geom_point(color = completions_airyards$team_color, size = 3.5) +
  ...
  theme(
    ...
    axis.title.x = element_text(size = 11, color = "#535353", family = "Roboto Condensed"),
    axis.ticks = element_blank()) +
  geom_text_repel(aes(label = name), family = "Roboto Condensed") +
  geom_hline(yintercept = mean(completions_airyards$mean_airyards), size = .8,
             color = "black", linetype = "dashed") +
  geom_vline(xintercept = mean(completions_airyards$completions), size = .8,
             color = "black", linetype = "dashed") +
  ylab("Mean Air Yards") +
  xlab("Total Completions") +
  labs(title = "Total Completions vs. Air Yards: 2021 Season")
```

![](_book/images/completed-passes-air-yards.png){fig-align="center"}

By adding a regression line to the plot using the `geom_smooth` function with the `method` set to "lm", we can see that there is a slight correlation between those quarterbacks with higher number of total completions and a lower average air yards. However, it is also reasonable to assume that both Justin Fields and Andy Dalton, as outliers, are skewing the results of the regression. Perhaps removing them from the data frame will provide a more realistic view of the relationship between total completions and average air yards.

```{r correlation-removing-adalton-jfields, echo = TRUE, eval = TRUE}

completions_airyards <- completions_airyards %>%
  filter(name != "J.Fields" & name != "A.Dalton")
