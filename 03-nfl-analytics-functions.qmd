...
The above code returns a result of 5,250 passing yards which is an exact match from the data provided by [Pro Football Reference.](https://www.pro-football-reference.com/players/M/MahoPa00.htm) In the above example, we used the `filter()` function on the `player_name` column in the data. You will get the same result if you replace that with `player_display_name == "Patrick Mahomes"`. While there is no difference in how the data is collected, there will be a difference if you were to visualize the data frame and wished to display the player names along with their respective statistics. In most cases, in order to maintain a clean design, using the `player_name` option is best which allows you to plot just the player's first initial and last name.

We can build upon the passing yards example to replicate the vast majority of statistics found in the["Passing" data table for Mahomes from Pro Football Reference](https://www.pro-football-reference.com/players/M/MahoPa00.htm#passing).
...
mahomes_adjusted_yards <- nflreadr::load_player_stats(seasons = 2018:2022) %>%
  ...
  group_by(season) %>%
  summarize(
    adj_yards_attempt = (sum(passing_yards) + 20 * sum(passing_tds) - 45 * sum(interceptions)) / sum(attempts))

mahomes_adjusted_yards
```

Based on the data output, Mahomes' best year for adjusted yards gained per pass attempt was 2018 with 9.58 adjusted yards. How does this compare to other quarterbacks from the same season? We can find the answer by doing slight modifications to our above code. Rather than filtering the information out to just Patrick Mahomes, we will add the `player_name` column as a second argument in the `group_by()` function.
...
all_qbs_adjusted_with_attempts <- nflreadr::load_player_stats(seasons = 2018) %>%
  filter(season_type == "REG" & position == "QB") %>%
  group_by(player_name) %>%
  summarize(
...
all_qbs_attempts_100 <- nflreadr::load_player_stats(seasons = 2018) %>%
  filter(season_type == "REG" & position == "QB") %>%
  group_by(player_name) %>%
  summarize(
    pass_attempts = sum(attempts),
...
best_adjusted_yards <- nflreadr::load_player_stats(seasons = 2018:2022) %>%
  filter(season_type == "REG" & position == "QB") %>%
...
```{r rushing-epa-leaders, eval = TRUE, echo = TRUE, output = TRUE}
...
receivers_wobr_ff_context <- nflreadr::load_player_stats(seasons = 2022) %>%
  filter(season_type == "REG" & position == "WR") %>%
  group_by(player_name) %>%
  summarize(
    total_wopr = sum(wopr),
...
The amount of information contained in the `nflverse` play-by-play data can be overwhelming. Luckily, the `nflfastR` website includes a searchable directory of all the variables with a brief description of what each one means. You can visit that here: [nflfastR Field Descriptions](https://www.nflfastr.com/articles/field_descriptions.html).
...
The following, with the first method using `group_by(passer)` and the second using `group_by(passer_player_name)` return differing results.
...
To examine the impact of "garbage time" statistics, we add provide an additional `filter()` that removes those plays that take place within the last 2-minutes of the half and when the probability of winning for either team is over 95% or under .05%.
...
The decision on range of the win probablity numbers is very much a personal preference. When `nflfastR` was first released, analyst often used a 20-80% range for the win probability cutoff point. However, Sebastian Carl - one of the creators of the `nflverse` explained in the package's Discord:
...
Brian Burke - prior to moving to ESPN - used to host his own blog where he showcased his analytics work. Way back in November of 2014, he wrote a post that detailed his process in determining how to value an offensive line's performance over the course of an NFL season.
...
pitt_line_value <- pbp %>%
  filter(sack == 1 | tackled_for_loss == 1 | yards_gained <= 2
         | qb_scramble == 1 | qb_hit == 1 | qbh_inc == 1 | qb_int == 1) %>%
  filter(posteam == "PIT") %>%
...
nfl_line_value <- pbp %>%
  ...
  filter(sack == 1 | tackled_for_loss == 1 | yards_gained <= 2
         | qb_scramble == 1 | qb_hit == 1 | qbh_inc == 1 | qb_int == 1) %>%
  left_join(nfl_plays, by = c("posteam" = "posteam", "week" = "week")) %>%
  group_by(week) %>%
...
A tool heavily used in the data vizualization process, the `load_teams()` function provides information pertaining to each NFL team's colors and logos, as well as providing a way to merge dataframes that have differing values for teams.

More often than not, we will be using `left_join` to bring in team informtion (colors, logos, etc.) into a another data frame. We will utilize this function heavily in the example below.
...
```{r qbr-adjusted-plot, output = TRUE, fig.align='center', warning = FALSE, fig.dpi = 400}
...
pfr_stats_pass <- nflreadr::load_pfr_advstats(seasons = 2022, stat_type = "pass",
...
```{r qb-pfr-stats-plot, output = TRUE, fig.align='center', warning = FALSE, fig.dpi = 400}
...
pfr_stats_rush <- nflreadr::load_pfr_advstats(seasons = 2022, stat_type = "rush",
                                              summary_level = "season") %>%
...
```{r pfr-rec-stats-plot, output = TRUE, fig.align='center', warning = FALSE, fig.dpi = 400}
...
pfr_stats_def <- nflreadr::load_pfr_advstats(seasons = 2022, stat_type = "def",
                                             summary_level = "season") %>%
...
```{r blitz-sacks-plot, output = TRUE, fig.align='center', warning = FALSE, fig.dpi = 400}
...
The `load_contracts()` function brings in data from [OverTheCap.com](https://overthecap.com/). It is important to remember that much of the information is "nested" and, if you wish to see the yearly information, you must use the `unnest()` function from `tidyr`. To highlight this, we can run the following code to bring in the data as it is stored in the `nflverse`.
...
```{r cash-paid-dakota-plot, output = TRUE, fig.align='center', warning = FALSE, fig.dpi = 400}

ggplot(data = nfl_contracts_unnested, aes(x = cash_paid, y = mean_dakota)) +
