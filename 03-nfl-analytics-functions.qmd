...
As one of the newest additions to the `nflverse`, users can use the `load_ftn_charting()` function to pull data compiled by the [FTN Network](https://twitter.com/FTNNetwork). The data is manually charted and includes the following variables:

1.  `starting_hash`
2.  `qb_location`
3.  `n_offense_backfield`
4.  `is_no_huddle`
5.  `is_motion`
6.  `is_play_action`
7.  `is_screen_pass`
8.  `is_rpo`
9.  `is_trick_play`
10. `is_qb_out_of_pocket`
11. `is_interception_worthy`
12. `is_throw_away`
13. `read_thrown`
14. `is_catchable_ball`
15. `is_contested_ball`
16. `is_created_reception`
17. `is_drop`
18. `is_qb_sneak`
19. `n_blitzers`
20. `n_pass_rushers`
...
pbp <- pbp %>%
  left_join(ftn_data, by = c("game_id" = "nflverse_game_id", "play_id" = "nflverse_play_id"))
