...
receivers_wopr_ff_context <- nflreadr::load_player_stats(seasons = 2022) %>%
  filter(season_type == "REG" & position == "WR") %>%
  group_by(player_name) %>%
  summarize(
    total_wopr = sum(wopr),
...
ggplot(data = ngs_data_passing, aes(x = avg_time_to_throw, y = avg_completed_air_yards)) +
  ...
  geom_text_repel(aes(label = player_display_name),
                  family = "Roboto Codensed", fontface = "bold", size = 3.5) +
  nfl_analytics_theme() +
...
ggplot(data = ngs_data_rushing, aes(x = expected_rush_yards, y = actual_rush_yards)) +
  ...
  scale_y_continuous(breaks = scales::pretty_breaks(),
                     labels = scales::comma_format()) +
  geom_text_repel(aes(label = player_display_name), family = "Roboto Codensed", fontface = "bold", size = 3.5) +
  nfl_analytics_theme() +
